...
def test_get_all_posts(authorized_client, test_posts):
    ...
    # def validate(post):
    #     return schemas.Pos(**post)
    # posts_map = map(validate, res.json())
    # res_list = list(posts_map)
    #assert res_list[0].Post.id == test_posts[0].id
    print("res:", res.json())
...
def test_get_one_post(authorized_client, test_posts):
    ...
    #print("res:", res.json())
    post = schemas.PostJoinVotes(**res.json())
    #print("post:", post)
...
def test_create_post_default_published_true(authorized_client, test_user):
    res = authorized_client.post(
        "/posts/",
        json={"title": "arbitrary title", "content": "some content"}
    )
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
...
def test_unauthrozied_user_delete_post(client, test_posts):
    res = client.delete(
